...
auxiliary_dict = {"a": {
                        ...
                        "IND;PST;PFV;LGSPEC1": {'1;SG': 'eux', '2;SG': 'eus', '3;SG': 'eut', '1;PL': 'eûmes',  '2;PL': 'eûtes',  '3;PL': 'eurent'},
...
def phonological_constrain_pronons(form, pron, type="a"):
    ...
    if type == "a":
        if re.match("^[aeiouy].*", form) and re.match(".*[aeiou]$", pron):
            return pron[:-1]+"'"
        else:
            return pron
    else:
        if re.match("^[aeiouy].*", form) and re.match(".*[aeiou]$", pron) and pron not in ["lui"]:
                ...
                for i, aspect in enumerate(aspects):
                    ...
                    for pn in person_number:
                        ...
                        for _response in response:
                            ...
                            # handling all datif and accusatif in same way for now
                            for _pron_feat, _pron in prepos_prons[_response].items():
                                ...
                                if mood == "IMP":
                                    ...
                                    full_form = seed_full_form + f"{nom_prons[pn]} {_pron_writing} {form} "
                                    ...
                                    if mood == "IND":
                                        if tense == "FUT":
                                            ...
                                            full_form = seed_full_form + f"{nom_prons[pn]} {_pron_writing_compound} {aux_form} {ptcp_pst}"
                                            new_table[full_feature] = full_form
                                            print(f"{nfin}\t{full_form}\t{full_feature}")
